...
const Track = sequelize.define('Track', {
  ...
  genre:      { type: DataTypes.STRING, defaultValue: 'Hard Dance' },
...
function parseFilename(name: string) {
  ...
  return { artist: 'Unknown', title: clean, version: '', versionType: 'clean' };
}

function parseBpmAndKey(name: string): { bpm: number; key: string } {
  const match = name.match(/([A-B]\d{1,2})\s*-\s*(\d{2,3})\.mp3$/i);
  if (match) return { key: match[1], bpm: parseInt(match[2]) };
  return { bpm: 0, key: '' };
}
...
async function main() {
  ...
  for (const f of files) {
    ...
    const { bpm, key } = parseBpmAndKey(f.name);
    ...
    try {
      ...
      console.log(`  ✅ ${artist} - ${title} [${version}] ${bpm}bpm ${key}`);
