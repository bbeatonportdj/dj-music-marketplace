...
async function main() {
  ...
  const [tracks] = await sequelize.query(`SELECT id, title FROM tracks WHERE genre = 'Hard Dance' AND title ~ ' - [0-9A-B]'`);
  console.log(`Found ${(tracks as any[]).length} titles with trailing key/bpm\n`);

  for (const t of tracks as any[]) {
    const stripped = t.title.replace(/\s*-\s*[A-B]\d{1,2}\s*-\s*\d{2,3}\s*$/, '').trim();
