...
const Track = sequelize.define('Track', {
  ...
  genre:      { type: DataTypes.STRING, defaultValue: 'Techno' },
...
function parseFilename(name: string) {
  ...
  // Try "Artist - Title (Version)" or "Artist - Title"
  ...
  // Try "Title (Version)" (no artist dash)
...
async function main() {
  ...
  // Get existing tracks for duplicate check
  ...
  for (const f of files) {
    // Skip if same gdrive_file_id already in DB
    ...
    // Skip duplicate within this folder
    ...
    // Check if same title+artist already exists
