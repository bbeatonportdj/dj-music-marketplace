...
const FOLDER_ID = '1snWlPSb1M-ReaM-TDwQfRUy2QHIjbQ6a';
...
const Track = sequelize.define('Track', {
  ...
  genre:      { type: DataTypes.STRING, defaultValue: 'Baile Funk/Favela Bass' },
...
function parseFilename(name: string) {
  ...
  // Version from brackets
  let version = '';
  let versionType = 'clean';
  const vm = clean.match(/\[(Intro(?:\s+(?:Clean|Dirty))?|Clean|Dirty|Extended|Instrumental)\]/i);
  if (vm) {
    version = vm[1];
    versionType = vm[1].toLowerCase().includes('dirty') ? 'dirty' : 'clean';
  }
...
async function getAllFiles(folderId: string): Promise<any[]> {
  ...
  const folderIds = folders.length > 0 ? folders.map((f: any) => f.id) : [folderId];
  ...
  for (const f of files) {
    ...
    try {
      const track = await Track.create({
        ...
        price: 0.60, gdrive_file_id: f.id, artwork_url: '',
